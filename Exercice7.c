...
int main(int argc , char* argv[])
{
    if(argc != 2)
    {
         printf("Usage: %s <addresse IP>\n", argv[0]);
        return 0;
    }

    char* Adress_Ip = argv[1];
    int sock;
    struct sockaddr_in target;

      printf("Scanner les ports  de %d a %d de  %s...\n", DEBUT_PORT, FIN_PORT,  Adress_Ip);
      ...
      for(int port= DEBUT_PORT ;port <=FIN_PORT ;port++)
      {
        ...
        target.sin_port = htons(port);
          ...
          if(inet_pton(AF_INET ,Adress_Ip, &target.sin_addr) == -1)
          {
            perror("Erreur de configuration de l'adresse ip\n");
            close(sock);
            return -1;
          }
